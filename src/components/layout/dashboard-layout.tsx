...
export default async function DashboardLayout({ children, title, badge }: DashboardLayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <DashboardHeader user={session.user} title={title} badge={badge} />
...
function DashboardHeader({ user, title, badge }: { user: any; title: string; badge?: string }) {
  return (
    <header className="bg-white border-b border-gray-200 px-6 py-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          {badge && (
            <Badge className="bg-blue-100 text-blue-800">{badge}</Badge>
          )}
        </div>
        <div className="flex items-center space-x-4">
